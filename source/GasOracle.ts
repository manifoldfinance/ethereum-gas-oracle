...
export class GasOracle {
  ...
  public getPercentile = async (percentile: number): Promise<number> => {
    ...
    if (sorted.length <= 0)
      throw new Error(`Please wait until the service has fetched at least one block.`);
  ...
  private getBlockByNumber = async (blockNumber: number | 'latest') => {
    let param = typeof blockNumber === 'number' ? `0x${blockNumber.toString(16)}` : blockNumber;
    const payload = {
      jsonrpc: '2.0',
      id: 1,
